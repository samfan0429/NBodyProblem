...
// #  endif
#  pragma acc data copy(curr[:n]) copyin(forces[:n][:1])
   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
//    memset(forces, 0, n*sizeof(vect_t));
#     pragma acc parallel loop independent copyin(curr[:n]) copy(forces[:n][:1])
      for (part = 0; part < n; part++){
         Compute_force(part, forces, curr, n);
      }   
// #     pragma omp for
#     pragma acc parallel loop independent copyout(curr[:n]) copyin(forces[:n][:1])
      for (part = 0; part < n; part++){
...
// #     endif
   ...
   printf("%e\t", finish-start); //Output print

...
void Compute_force(vect_t forces[], struct particle_s curr[], 
      int n);
void Update_part(vect_t forces[], struct particle_s curr[], 
...
// #     pragma omp for
   // for (part = 0; part < n; part++){
      Compute_force(forces, curr, n);
   // }   
// #     pragma omp for
      // for (part = 0; part < n; part++)
      Update_part(forces, curr, n, delta_t);
// #     ifndef NO_OUTPUT
// #     pragma omp single 
...
#  endif
#  pragma acc parallel private(mg, f_part_k, len, len_3, fact)
{
...
void Update_part(vect_t forces[], struct particle_s curr[], 
      int n, double delta_t) {
   double fact;
#  pragma acc parallel loop private(fact)// reduction(+:curr[part])   
   for (int part = 0; part < n; part++){
      fact = delta_t/curr[part].m;
      #  ifdef DEBUG
...
// #  pragma acc parallel loop private(speed_sqr) reduction(ke)
   for (int i = 0; i < n; i++) {
      speed_sqr = curr[i].v[X]*curr[i].v[X] + curr[i].v[Y]*curr[i].v[Y];
      ke += curr[i].m*speed_sqr;
   }
   ke *= 0.5;
// #  pragma acc parallel loop private(dist) reduction(pe)
   for (int i = 0; i < n-1; i++) {
      for (int j = i+1; j < n; j++) {

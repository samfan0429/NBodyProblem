...
//       shared(curr, forces, thread_count, delta_t, n, n_steps, output_freq,step,t) \
//       private(part)
   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
//    memset(forces, 0, n*sizeof(vect_t));
#     pragma acc kernels //shared(curr, forces,n) private(part)
      {
#     pragma acc loop independent        
      for (part = 0; part < n; part++){
         Compute_force(part, forces, curr, n);
      }   
// #     pragma omp for //shared(curr, forces, n , delta_t) private(part)
#     pragma acc loop independent
      for (part = 0; part < n; part++){
